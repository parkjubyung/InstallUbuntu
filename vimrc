...
" " set paste가 설정되어 있지 않으면 vim 에서 붙여넣기 할 때 매 라인들이
" 들여쓰기가 된다.
...
set expandtab " 탭을 공백으로 변환한다. 만일 Makefile 등에서 탭이 필요한
" 경우에는 :se noet
"
" " 이렇게 설정했는데도 불구하고 탭이 있다면 붉은색으로 경고한다.
:"match ErrorMsg '	' " 이렇게 설정했는데도 불구하고 탭이 있다면 노랜색으로
" 경고한다.
"
" " Makefile 등 의도한 탭이 아니라면 탭을 스페이스로 바꾸자.
map ,<TAB> :%s/	/    /g<CR>
" 콤마(,)와 <TAB>을 순서대로 누르면 탭을
" 스페이스로 변경.
...
map <C-c> :<ESC>i<SPACE>/**<<SPACE><SPACE>*/<ESC>
"map <C-x> :Dox<CR>
map <C-x> i// TODO: <ESC>
...
nmap <S-Left> <C-W>l<C-W>\|
nmap <C-Right> <C-W>><C-W>><C-W>><C-W>>
nmap <S-Right> <C-W>h<C-W>\|
nmap <A-Right> <C-W>\|
